 ...
 * @param {boolean} includeTest - If true, include test brands (default: false)
 */
export async function getBrands(includeTest = false) {
  const collection = await getCollection('brands');
  const query = includeTest ? {} : { isTest: { $ne: true } };
  const brands = await collection.find(query).sort({ id: 1 }).toArray();
 ...
 * Excludes test brands from sitemap
 */
export async function getAllVehicles() {
  const [brands, models, types, engines] = await Promise.all([
    getCollection('brands').then(c => c.find({ isTest: { $ne: true } }).toArray()),
